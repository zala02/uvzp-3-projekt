...
class PrepareData:
    ...
    def _load_and_process(self):
        ...
        # for one attribute we also take the closest time 
        # but only if that time is maximu 5 hours back otherwise we drop it
        for station in self.station_columns:
            best_values = [np.nan] * len(data)
            for i in range(len(data)):
                ...
                for lag in range(1, max_lag_hours + 1):
                    ...
                    if j < 0:
                        break
                    time_diff = (data.loc[i, 'timestamp'] - data.loc[j, 'timestamp']).total_seconds() / 3600
                    if time_diff == lag and not np.isnan(data.loc[j, station]):
                        best_values[i] = data.loc[j, station]
                        found = True
                        break
                # If no value found, it stays NaN
            col_name = f'prev_station_{station}'
            data[col_name] = best_values
            lag_features.append(col_name)

        data = data.dropna(subset=lag_features).reset_index(drop=True)

        self.attributes += lag_features
        self.station_columns = data.columns.difference(['timestamp'] + self.attributes)
...
class Model_Neural_Network(nn.Module):
    ...
    def __init__(self, input_dim=123, num_stations=84, lr=0.001, epochs=1500, lambda_reg=0.00001):
        super().__init__()
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            ...
            features = ['hour_0', 'hour_1', 'hour_2', 'hour_3', 'hour_4', 'hour_5', 'hour_6', 'hour_7', 'hour_8', 'hour_9', 'hour_10', 'hour_11', 'hour_12', 'hour_13', 'hour_14', 'hour_15', 'hour_16', 'hour_17', 'hour_18', 'hour_19', 'hour_20', 'hour_21', 'hour_22', 'hour_23', 'month_1', 'month_2', 'month_3', 'month_4', 'month_5', 'month_6', 'month_7', 'month_8', 'month_9', 'month_10', 'month_11', 'month_12', 'is_weekend', 'is_holiday', 'is_school_holiday', 'prev_station_ALEJA - CELOVŠKA CESTA', 'prev_station_AMBROŽEV TRG', 'prev_station_ANTONOV TRG', 'prev_station_BARJANSKA C.-CENTER STAREJŠIH TRNOVO', 'prev_station_BAVARSKI DVOR', 'prev_station_BONIFACIJA', 'prev_station_BRATOVŠEVA PLOŠČAD', 'prev_station_BREG', 'prev_station_BRODARJEV TRG', 'prev_station_BS4-STOŽICE', 'prev_station_BTC CITY ATLANTIS', 'prev_station_BTC CITY/DVORANA A', 'prev_station_CANKARJEVA UL.-NAMA', 'prev_station_CESTA NA ROŽNIK', 'prev_station_CITYPARK', 'prev_station_DOLENJSKA C. - STRELIŠČE', 'prev_station_DRAVLJE', 'prev_station_DUNAJSKA C.-PS MERCATOR', 'prev_station_DUNAJSKA C.-PS PETROL', 'prev_station_GERBIČEVA - ŠPORTNI PARK SVOBODA', 'prev_station_GH ŠENTPETER-NJEGOŠEVA C.', 'prev_station_GRUDNOVO NABREŽJE-KARLOVŠKA C.', 'prev_station_HOFER - POLJE', 'prev_station_HOFER-KAJUHOVA', 'prev_station_IKEA', 'prev_station_ILIRSKA ULICA', 'prev_station_KINO ŠIŠKA', 'prev_station_KONGRESNI TRG-ŠUBIČEVA ULICA', 'prev_station_KOPALIŠČE KOLEZIJA', 'prev_station_KOPRSKA ULICA', 'prev_station_KOSEŠKI BAJER', 'prev_station_LEK - VEROVŠKOVA', 'prev_station_LIDL - RUDNIK', 'prev_station_LIDL - VOJKOVA CESTA', 'prev_station_LIDL BEŽIGRAD', 'prev_station_LIDL-LITIJSKA CESTA', 'prev_station_MASARYKOVA DDC', 'prev_station_MERCATOR CENTER ŠIŠKA', 'prev_station_MERCATOR MARKET - CELOVŠKA C. 163', 'prev_station_MIKLOŠIČEV PARK', 'prev_station_P + R DOLGI MOST', 'prev_station_P+R BARJE', 'prev_station_PARK NAVJE-ŽELEZNA CESTA', 'prev_station_PARKIRIŠČE NUK 2-FF', 'prev_station_PLEČNIKOV STADION', 'prev_station_POGAČARJEV TRG-TRŽNICA', 'prev_station_POLJANSKA-POTOČNIKOVA', 'prev_station_POLJE', 'prev_station_POVŠETOVA - KAJUHOVA', 'prev_station_POVŠETOVA-GRABLOVIČEVA', 'prev_station_PREGLOV TRG', 'prev_station_PREŠERNOV TRG-PETKOVŠKOVO NABREŽJE', 'prev_station_PRUŠNIKOVA', 'prev_station_RAKOVNIK', 'prev_station_ROŠKA - STRELIŠKA', 'prev_station_ROŽNA DOLINA-ŠKRABČEVA UL.', 'prev_station_SAVSKO NASELJE 1-ŠMARTINSKA CESTA', 'prev_station_SAVSKO NASELJE 2-LINHARTOVA CESTA', 'prev_station_SITULA', 'prev_station_SOSESKA NOVO BRDO', 'prev_station_SREDNJA FRIZERSKA ŠOLA', 'prev_station_STARA CERKEV', 'prev_station_STUDENEC', 'prev_station_SUPERNOVA LJUBLJANA - RUDNIK', 'prev_station_TEHNOLOŠKI PARK', 'prev_station_TIVOLI', 'prev_station_TRG MDB', 'prev_station_TRG OF-KOLODVORSKA UL.', 'prev_station_TRNOVO', 'prev_station_TRŽAŠKA C.-ILIRIJA', 'prev_station_TRŽNICA KOSEZE', 'prev_station_TRŽNICA MOSTE', 'prev_station_VILHARJEVA CESTA', 'prev_station_VIŠKO POLJE', 'prev_station_VOJKOVA - GASILSKA BRIGADA', 'prev_station_VOKA - SLOVENČEVA', 'prev_station_ZALOG', 'prev_station_ZALOŠKA C.-GRABLOVIČEVA C.', 'prev_station_ČRNUČE', 'prev_station_ŠMARTINSKI PARK', 'prev_station_ŠPICA', 'prev_station_ŠPORTNI CENTER STOŽICE', 'prev_station_ŠTEPANJSKO NASELJE 1-JAKČEVA ULICA', 'prev_station_ŽIVALSKI VRT']
...
def predict_and_fill_test_gaps(test_path, model, output_path="bicikelj_predictions.csv"):
    ...
    df = add_holiday_features(df)  # adds is_holiday, is_school_holiday
    ...
    for col in expected_hours:
        if col not in df.columns:
            df[col] = 0
    for col in expected_months:
        if col not in df.columns:
            df[col] = 0


    # Define base attributes and station columns
    ...
    # Create lag features from most recent non-NaN value within last 5 hours
    lag_features = []
    for station in station_columns:
        lag_col = f'prev_station_{station}'
        best_values = []

        for i in range(len(df)):
            best_val = np.nan
            for j in range(1, 6):  # try up to 5 previous rows
                if i - j < 0:
                    break
                delta_hours = (df.loc[i, 'timestamp'] - df.loc[i - j, 'timestamp']).total_seconds() / 3600
                if delta_hours > 5:
                    break
                val = df.loc[i - j, station]
                if not np.isnan(val):
                    best_val = val
                    break
            best_values.append(best_val)

        df[lag_col] = best_values
        lag_features.append(lag_col)

    # Combine all attributes
    full_attributes = base_attributes + lag_features

    # Fill missing attributes (just in case model was trained with more)
    ...
    for idx in range(len(df)):
        if df.loc[idx, station_columns].isna().all():
            # Predict from current row's attributes
            X_row = df.loc[[idx], full_attributes].values.astype(np.float32)  # shape: (1, input_dim)
            X_scaled = scaler.transform(X_row)
            X_tensor = torch.tensor(X_scaled, dtype=torch.float32)

            with torch.no_grad():
                y_pred = model(X_tensor).squeeze(0).numpy()  # shape: (num_stations,)

            df_filled.loc[idx, station_columns] = y_pred
    ...
    df_output['timestamp'] = df_output['timestamp'].dt.strftime('%Y-%m-%dT%H:%MZ')
