...
class PrepareData:
    ...
    def _load_and_process(self):
        ...
        self.attributes = ['hour', 'is_weekend', 'is_holiday', 'is_school_holiday']
        cols_to_drop = ['timestamp'] + self.attributes
        self.station_columns = [col for col in data.columns if col not in cols_to_drop]
...
class Model_Linear_Regression(nn.Module):
    ...
    def __init__(self, input_len=48, input_dim=4, output_len=4, num_stations=84, lr=0.001, epochs=1000):
        super().__init__()
        # Model: simple linear layer
        self.linear = nn.Linear(input_len * input_dim, output_len * num_stations)
    ...
    def forward(self, x):
        x = self.flatten(x)  # shape: [batch_size, 48 * 2]
        x = self.linear(x)   # shape: [batch_size, 4 * 84]
        x = x.view(-1, self.output_len, self.num_stations)  # shape: [batch_size, 4, 84]
        return x
    ...
    def fit(self, train_loader):
        ...
        for epoch in range(self.epochs):
            ...
            for xb, yb in train_loader:
                ...
                #    if 'weight' in name:
                self.soft_threshold(self.linear.weight, self.lambda_reg)
    ...
    def predict(self, X):
        ...
        with torch.no_grad():
            ...
            features = ['hour', 'is_weekend', 'is_holiday', 'is_school_holiday']
...
def predict_and_fill_test_gaps(test_path, model, output_path="bicikelj_predictions.csv"):
    ...
    attributes = ['hour', 'is_weekend', 'is_holiday', 'is_school_holiday']
    ...
    while i + TRAIN_SIZE + PREDICT_SIZE <= len(df):
        ...
        if future[station_columns].isna().all().all():
            X_input = hist[attributes].values.astype(np.float32)
            X_input_tensor = torch.tensor(X_input).unsqueeze(0)  # shape: (1, 48, num_features)

            # Predict
            with torch.no_grad():
                y_pred = model(X_input_tensor).squeeze(0).numpy()  # shape: (4, num_stations)
...
# just delete standardization for better results
# regularization is fine but use 0.00001 lambda and we get MSE = 25 yay.