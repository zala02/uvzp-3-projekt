...
class Model_Neural_Network(nn.Module):
    ...
    def __init__(self, input_dim=39, num_stations=84, lr=0.001, epochs=1500, lambda_reg=0.00001):
        super().__init__()
...
def predict_and_fill_test_gaps(test_path, model, output_path="bicikelj_predictions.csv"):
    ...
    # Final attributes used in training
    attributes = expected_hours + expected_months + ['is_weekend', 'is_holiday', 'is_school_holiday']
    ...
    for idx in range(len(df)):
        if df.loc[idx, station_columns].isna().all():
            ...
            X_row = df.loc[[idx], attributes].values.astype(np.float32)  # shape: (1, input_dim)
